...
export default function MovimientosRecientes() {
  ...
  async function loadAll() {
    ...
    if (profileData?.[0] && !profileId) setProfileId(profileData.find(p => p.role === 'mama' || p.role === 'papa')?.id ?? '')
    setLoading(false)
  }
  ...
  async function submit() {
    ...
    setError('')
    setSaving(true)
    const { start: todayStart } = quincenaFromIndex(currentQuincenaIndex())
    ...
    await supabase.from('transactions').insert({
      ...
      quincena_start: toISODateString(todayStart),
    })
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-1.5 mt-1 flex-wrap items-center">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-purple-100 text-purple-700">{card?.name ?? 'Efectivo'}</span>
                    {t.needs_invoice && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700">Facturar</span>}
                    {(t.tags ?? []).map(tag => (
